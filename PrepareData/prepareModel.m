...
for i = 1 : invpar.nModels
    ...
    switch invpar.model{i}
        ...
        case 'PCDM'
            nParameters = 9;
            index2 = index1 + nParameters - 1;
            [~,ii]=ismember(i,find(strcmp(invpar.model,'PCDM'))); % Find which number geometry that we are on, and take that
            model.m(index1:index2) = modelInput.pcdm{ii}.start;
            model.step(index1:index2) = modelInput.pcdm{ii}.step;
            model.lower(index1:index2) = modelInput.pcdm{ii}.lower;
            model.upper(index1:index2) = modelInput.pcdm{ii}.upper;
            model.gaussPrior(index1:index2) = false(nParameters,1);
            model.modelName(index1:index2)={['PCDM',num2str(ii)]};
